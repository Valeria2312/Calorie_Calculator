...
const updateCalories = () => {
    allCalories = arrProducts.reduce((total, item) => total + Number(item.calories), 0);
...
const warningsDailyLimit = () => {
  if (allCalories > totalCaloriesDay) {
      warningsDaily.textContent = "Вы привысили дневную норму";
...
const filterItems = (e) => {
    ...
    const filterArr = arrProducts.filter((item) => {return item.product.indexOf(filter) !== -1})
    listItems.innerHTML = '';
